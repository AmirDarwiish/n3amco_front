  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      {Object.entries(grouped).map(([module, perms]) => {
        ...
        return (
          <div key={module} style={{ background:'var(--bg-base)', border:'1px solid var(--border)', borderRadius:10, overflow:'hidden' }}>
            <div
              onClick={() => toggleModule(perms)}
              style={{ padding:'9px 14px', borderBottom:'1px solid var(--border)', background:'var(--bg-card)', display:'flex', alignItems:'center', gap:10, cursor:'pointer', userSelect:'none' }}
            >
              <input
                type="checkbox"
                checked={allOn}
              ...
              />
              <span style={{ fontSize:12, fontWeight:800, color:'var(--gold)' }}>{moduleName(module)}</span>
              ...
              {perms.map(p => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <span style={{ fontSize:12, color: isOn ? 'var(--text)' : 'var(--text-muted)', lineHeight:1.3 }}>
                      {permName(code)}
                    </span>
                  </label>
                )
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap' }}>
        ...
        />
        <button style={S.btnGhost} onClick={() => navigate('/dashboard/reports/activity')}>تقارير النشاط</button>
        <button style={S.btnGold}  onClick={() => openModal('create')}>+ مستخدم جديد</button>
      </div>

      <div style={S.card}>
        {loading
          ? <div style={{ padding:40, textAlign:'center', ...S.muted }}>جاري التحميل...</div>
          : filtered.length === 0
            ? <div style={{ padding:40, textAlign:'center', ...S.muted }}>لا توجد نتائج</div>
            : filtered.map(u => (
              <div key={u.id}
                style={{ ...S.row }}
              ...
              >
                <div style={{ display:'flex', alignItems:'center', gap:12, minWidth:0, flex:1 }}>
                  <div style={{
                    ...
                    color: u.isActive ? 'var(--gold)' : 'var(--text-muted)', fontWeight:800, fontSize:14,
                  }}>
    ...
    <Modal title={`تعديل: ${user?.fullName}`} onClose={onClose}>
      <div style={{ display:'flex', flexDirection:'column', gap:13 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          />
          <label htmlFor="isActive" style={{ fontSize:13, color:'var(--text-muted)', cursor:'pointer' }}>الحساب نشط</label>
        </div>
        {error && <div style={S.err}>{error}</div>}
        <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
          <button onClick={onClose} style={S.btnGhost}>إلغاء</button>
...
function ChangeRoleModal({ user, roles, onClose, onSuccess }) {
  ...
  return (
    <Modal title={`تغيير دور: ${user?.fullName}`} onClose={onClose} maxWidth={400}>
      <div style={{ display:'flex', flexDirection:'column', gap:13 }}>
        <div style={{ background:'var(--bg-base)', border:'1px solid var(--border)', borderRadius:8, padding:'10px 14px', fontSize:12, color:'var(--text-muted)' }}>
          الدور الحالي:&nbsp;
    ...
    <Modal title={`تغيير كلمة المرور: ${user?.fullName}`} onClose={onClose} maxWidth={380}>
      <div style={{ display:'flex', flexDirection:'column', gap:13 }}>
        <div style={{ position:'relative' }}>
          ...
          <button
            onClick={() => setShow(s => !s)}
            style={{ position:'absolute', left:10, bottom:9, background:'none', border:'none', color:'var(--text-muted)', cursor:'pointer', padding:0, fontSize:13 }}
          >
            {show ? '🙈' : '👁️'}
          </button>
        </div>
    ...
    <div>
      ...
      <div style={S.card}>
        {loading
          ? <div style={{ padding:40, textAlign:'center', ...S.muted }}>جاري التحميل...</div>
          : roles.length === 0
            ? <div style={{ padding:40, textAlign:'center', ...S.muted }}>لا توجد أدوار</div>
            : roles.map(r => (
              <div key={r.id}
                style={{ ...S.row }}
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(201,169,110,.04)'}
              ...
              >
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  {r.usersCount !== undefined && (
                    <span style={S.tag('var(--text-muted)')}>{r.usersCount} مستخدم</span>
                  )}
                </div>
  ...
  return (
    <Modal title={`تعديل الدور: ${role?.name}`} onClose={onClose} maxWidth={640}>
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        ...
        <div style={{ background:'var(--bg-base)', border:'1px solid var(--border)', borderRadius:8, padding:'8px 14px', fontSize:12, color:'var(--gold)', display:'flex', justifyContent:'space-between' }}>
        ...
        <div style={{ maxHeight:420, overflowY:'auto' }}>
          {fetching
            ? <div style={{ ...S.muted, padding:24, textAlign:'center' }}>جاري التحميل...</div>
            : <PermissionCheckboxes all={allPerms} selected={selected} onChange={setSelected} />
          }
        </div>
