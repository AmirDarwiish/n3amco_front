...
<section id="products" style={{ padding: '88px 0', background: '#fafaf7', overflow: 'hidden' }}>
  ...
  {loadingData ? (
    ...
    />
      ...
      {/* ════ WAITLIST ════ */}
      <section id="waitlist" style={{ padding: '88px 24px', background: '#fafaf7' }}>
        <div style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center' }}>
          <div className="card waitlist-card" style={{ padding: '46px 40px', borderColor: 'rgba(184,132,58,0.25)', background: 'linear-gradient(160deg, #fff 60%, #fdf8f0)', boxShadow: '0 8px 40px rgba(28,25,23,0.07)' }}>
            <div className="chip" style={{ marginBottom: 16 }}><span className="chip-dot" /> قائمة الانتظار</div>
            <h2 style={{ fontSize: 24, fontWeight: 900, color: '#1c1917', marginBottom: 10 }}>متفوتكش الدفعة الجاية</h2>
            <p style={{ fontSize: 13, color: '#78716c', marginBottom: 26, lineHeight: 1.85 }}>
            ...
            {wlJoined ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <input className="n3-input" placeholder="اسمك"   value={wlName}  onChange={e => setWlName(e.target.value)}  style={{ flex: 1, height: 46, minWidth: 140 }} />
                  <input className="n3-input" placeholder="موبايلك" value={wlPhone} onChange={e => setWlPhone(e.target.value)} style={{ flex: 1, height: 46, minWidth: 140 }} dir="ltr" />
                </div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <input className="n3-input" type="number" placeholder="كمية تقريبية (كجم)" value={wlKg} onChange={e => setWlKg(e.target.value)} style={{ flex: 1, height: 46, minWidth: 140 }} />
                  <button className="btn-gold" onClick={handleWaitlist} disabled={wlLoading} style={{ height: 46, padding: '0 22px', fontSize: 13, flexShrink: 0 }}>
                    {wlLoading ? <Icons.loading /> : 'انضم للقائمة'}
                  </button>
                </div>
                {wlError && <div style={{ fontSize: 12, color: '#dc2626', fontWeight: 700, textAlign: 'right' }}>{wlError}</div>}
              </div>
            )}
          </div>
        </div>
